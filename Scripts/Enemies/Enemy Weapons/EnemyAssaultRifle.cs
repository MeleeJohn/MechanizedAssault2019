...
public class EnemyAssaultRifle : MonoBehaviour {
    ...
    public GameObject enemyAssaultBullet;
    public int Ammo = 80;
    ...
    void Start() {
        ...
        for (int i = 0; i < ammoReference + extraAmmo; i++) {
            GameObject enemyAR_Bullet = Instantiate(enemyAssaultBullet, bulletPoolParent.transform);
            bulletPool.Add(enemyAR_Bullet);
    ...
    void Shoot() {
        ...
        if (RE.distanceToPlayer > 20f) {
            if (bulletPool[poolCount] == null) {
                GameObject Bullet = Instantiate(enemyAssaultBullet);
            ...
            bulletPool[poolCount].GetComponent<Rigidbody>().AddForce(-ShotSpawn.transform.right * 500f, ForceMode.VelocityChange);
